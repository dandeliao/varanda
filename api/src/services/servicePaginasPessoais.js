...
async function updateBlocosPaginaPessoal (html, pagina_pessoal_id) {
	...
	// lê html e captura lista de blocos com seus bloco_id (do nome da tag) e bloco_pagina_pessoal_id (do atributo "v_id")
...
// edita html e adiciona propriedade v_id="${bloco_pagina_pessoal_id}" para cada bloco que não a tem
async function updateHtmlBlocos(html, blocos) {
	...
	for (let i = 0; i < blocos.length; i++) {
		...
		if (b.jaTem === false || null || undefined) {
			b.tag = b.tag.replace(`${b.bloco_id}`,`${b.bloco_id} v_id="${b.bloco_pagina_pessoal_id}"`);
		}
