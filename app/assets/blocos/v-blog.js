...
class VBlog extends VarandaElement {
    constructor() {
    ...
    async renderizar(estado, noEvent) {
		...
		if (textos.length > 0) {
			...
			textos.forEach(async txt => {
				...
				let elTexto = document.createElement('v-texto');
				let textoId;
				if (tipo === 'pessoa') {
				...
				if (tipo === 'comunidade') { // posts em perfis pessoais não têm comentários, só posts em comunidades
					...
					if (!noEvent) {
						elBotaoComentarios.addEventListener('click', async e => {
							...
							// encontra o bloco v-texto parente do botão
							const textos = this.querySelectorAll('v-texto');
...
window.customElements.define('v-blog', VBlog);