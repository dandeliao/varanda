...
class VAlbum extends VarandaElement {
    constructor() {
    ...
    async renderizar(estado, noEvent) {
		...
		// cria cabeçalho
		let blocoCabecalho = document.createElement('v-bloco');
		...
		if (imagens.length > 0) {
			...
			imagens.forEach(async img => {
				...
				let elImg = document.createElement('v-imagem');
				let imagemId;
				if (tipo === 'pessoa') {
				...
				if (tipo === 'comunidade') { // posts em perfis pessoais não têm comentários, só posts em comunidades
					...
					if (!noEvent) {
						elBotaoComentarios.addEventListener('click', async e => {
							...
							// encontra o bloco v-imagem parente do botão
							const imagens = this.querySelectorAll('v-imagem');
...
window.customElements.define('v-album', VAlbum);